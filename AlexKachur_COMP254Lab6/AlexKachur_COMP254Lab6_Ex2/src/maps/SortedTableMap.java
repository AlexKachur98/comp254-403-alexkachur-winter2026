 ...
 *
 * MODIFIED for Lab 6 Exercise 2: the containKey(k) method is now fully
 * implemented using the existing findIndex(k) helper, resolving the ambiguity
 * of get(k) returning null for both "key absent" and "key mapped to null".
 *
...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
   ...
   * Resolves the ambiguity of get(k) returning null, which could otherwise
   * mean either "key not found" or "key mapped to null value".
   *
   * Uses the existing findIndex(k) method, as hinted in the assignment.
   *
   * @param key  the key whose presence is to be tested
   * @return true if an entry with this key exists; false otherwise
   */
  public boolean containKey(K key) throws IllegalArgumentException {
    checkKey(key);                                     // safe key check
    int j = findIndex(key);                            // binary search
    // j must be valid AND the entry at j must actually match the key
