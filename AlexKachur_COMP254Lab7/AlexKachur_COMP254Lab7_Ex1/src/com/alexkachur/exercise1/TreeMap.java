...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  // =====================================================================
  // EXERCISE 1: Iterative treeSearch - Alex Kachur
  // =====================================================================
  //
  // The textbook's treeSearch (above) uses recursion. On a very tall,
  // unbalanced BST the recursion depth can exceed Java's call-stack limit
  // (typically ~10k-20k frames) and throw a StackOverflowError.
  //
  // This alternative implementation uses a while-loop instead of recursive
  // calls, so its memory usage is O(1) regardless of tree height. It
  // survives arbitrarily deep trees as long as they fit in the heap.
  //
  // Semantics are identical to the recursive version:
  //   - if the key is found, return that node's Position
  //   - if not, return the terminal (external/sentinel) leaf reached
  // =====================================================================
  private Position<Entry<K,V>> treeSearchIterative(Position<Entry<K,V>> p, K key) {
    Position<Entry<K,V>> walk = p;                // start at the subtree root
    while (!isExternal(walk)) {                   // stop when we hit a sentinel leaf
    ...
    return walk;                                  // key not found; return terminal leaf
  }

  // Public wrapper so the test driver can exercise the iterative version.
  // Mirrors the textbook's get() but uses treeSearchIterative internally.
  public V getIterative(K key) throws IllegalArgumentException {
    checkKey(key);
    Position<Entry<K,V>> p = treeSearchIterative(root(), key);
    rebalanceAccess(p);
...
public static void main(String[] args) {
    System.out.println("========================================================");
    System.out.println("  LAB 7 EXERCISE 1 - Iterative treeSearch");
    System.out.println("  Alex Kachur");
    System.out.println("========================================================");

    // --------------------------------------------------------------------
    // TEST 1: Small balanced-ish tree - verify both versions find same keys
    // --------------------------------------------------------------------
    System.out.println("\n--- TEST 1: Small tree, compare recursive vs iterative ---");
    TreeMap<Integer,String> map = new TreeMap<>();
    int[] smallKeys = {50, 30, 70, 20, 40, 60, 80, 10, 35, 65, 90};
    String[] smallVals = {"fifty","thirty","seventy","twenty","forty","sixty",
                          "eighty","ten","thirty-five","sixty-five","ninety"};
    for (int i = 0; i < smallKeys.length; i++)
      map.put(smallKeys[i], smallVals[i]);

    System.out.println("Inserted " + smallKeys.length + " entries.");
    map.dump();

    // Verify every inserted key can be found by both methods with the same result
    boolean allMatch = true;
    for (int k : smallKeys) {
      String recResult = map.get(k);               // uses recursive treeSearch internally
      String iterResult = map.getIterative(k);     // uses iterative treeSearch
      String tag = recResult != null && recResult.equals(iterResult) ? "PASS" : "FAIL";
      if (!tag.equals("PASS")) allMatch = false;
      System.out.printf("  key=%-3d  recursive=%-12s  iterative=%-12s  [%s]%n",
                        k, recResult, iterResult, tag);
    }

    // Verify missing keys also behave the same (both return null)
    int[] missing = {1, 55, 100, 999};
    for (int k : missing) {
      String recResult = map.get(k);
      String iterResult = map.getIterative(k);
      String tag = (recResult == null && iterResult == null) ? "PASS" : "FAIL";
      if (!tag.equals("PASS")) allMatch = false;
      System.out.printf("  missing key=%-3d  recursive=%-5s  iterative=%-5s  [%s]%n",
                        k, recResult, iterResult, tag);
    }
    System.out.println(allMatch ? "TEST 1 PASS: all lookups agree" : "TEST 1 FAIL");

    // --------------------------------------------------------------------
    // TEST 2: Deeply unbalanced tree - both methods find the deepest key
    // --------------------------------------------------------------------
    System.out.println("\n--- TEST 2: Deep unbalanced tree (right-leaning) ---");
    TreeMap<Integer,Integer> deepMap = new TreeMap<>();
    int N = 2000;  // safely within JVM default stack so put() doesn't overflow
    System.out.println("Inserting " + N + " sequential keys 1..N. Because keys arrive");
    System.out.println("in sorted order the tree leans all the way right, giving depth ~" + N + ".");
    for (int i = 1; i <= N; i++)
      deepMap.put(i, i * 10);
    System.out.println("Tree built. size = " + deepMap.size());

    Integer recDeep = deepMap.get(N);                  // recursive treeSearch path
    Integer iterDeep = deepMap.getIterative(N);        // iterative treeSearch path
    boolean deepMatch = recDeep != null && recDeep.equals(iterDeep) && iterDeep == N * 10;
    System.out.println("  get(" + N + ")          = " + recDeep);
    System.out.println("  getIterative(" + N + ") = " + iterDeep);
    System.out.println("  " + (deepMatch ? "PASS: both methods agree at depth " + N
                                         : "FAIL"));

    // --------------------------------------------------------------------
    // TEST 2b: Why the iterative version matters - empirical stack limit
    // --------------------------------------------------------------------
    System.out.println("\n--- TEST 2b: How deep can Java recurse before StackOverflowError? ---");
    int measured = measureStackDepth();
    System.out.println("  Empirically measured JVM recursion limit: ~" + measured + " frames");
    System.out.println("  At tree depth beyond this, the recursive treeSearch would throw");
    System.out.println("  StackOverflowError. The iterative version uses O(1) stack and is");
    System.out.println("  immune to this limit - it only needs the heap.");

    // --------------------------------------------------------------------
    // TEST 3: Edge cases
    // --------------------------------------------------------------------
    System.out.println("\n--- TEST 3: Edge cases for iterative treeSearch ---");

    // Empty map
    TreeMap<Integer,String> empty = new TreeMap<>();
    System.out.println("  empty map getIterative(5) = " + empty.getIterative(5)
                       + "   [" + (empty.getIterative(5) == null ? "PASS" : "FAIL") + "]");

    // Single-node map
    TreeMap<Integer,String> single = new TreeMap<>();
    single.put(42, "answer");
    System.out.println("  single-node getIterative(42)  = " + single.getIterative(42)
                       + "   [" + ("answer".equals(single.getIterative(42)) ? "PASS" : "FAIL") + "]");
    System.out.println("  single-node getIterative(99)  = " + single.getIterative(99)
                       + "   [" + (single.getIterative(99) == null ? "PASS" : "FAIL") + "]");
  ...
  // ---- helper: measure Java's recursion depth before StackOverflowError ----
  private static int measureStackDepth() {
    int[] counter = new int[1];                 // box so the recursive call can update it
    try {
      recurse(counter);
    } catch (StackOverflowError e) {
      // swallow - we expected it
    }
    return counter[0];
  }

  private static void recurse(int[] counter) {
    counter[0]++;
    recurse(counter);                           // each call adds one frame
  }
}
